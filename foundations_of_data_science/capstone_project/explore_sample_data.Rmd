...
loans <- read_csv('./dataset/loans.csv')
```
...
# assuming loans w/o partner_id are direct loans?
direct_loans <- loans[is.na(loans$partner_id), ]
direct_loans$location.country_code # kiva site says only in US, YES
loans$partner <- as.numeric(is.na(loans$partner_id))

# maybe borrower count has implication
loans$group <- as.numeric(loans$borrower_count > 1)

# there are a few large borrowers with funded amount of USD 50k
# remove them for now
large_borrowers <- loans[loans$funded_amount >= 50000, ]
loans <- loans[!loans$funded_amount >= 50000, ]
```
...
### What is the distribution of funded amounts?

```{r}
ggplot(loans, aes(x=funded_amount)) + geom_histogram(binwidth = 100)
```
Right skewed bar plot. Most loans that receive funding are small.



### Any Relation between lender count and funded amount?

Is there a group effect on the lenders with regard to the loan amounts?

```{r}
ggplot(loans, aes(x=lender_count, y=funded_amount)) + geom_jitter()
```

The higher the loan amount is the more number of people who fund it are. Lower loan amounts attract varied number of lenders though.

### Lender vs Borrower

Are lenders biased against many borrowers or individual lenders?
...
### Borrower_count vs funded_amount

Do groups (multiple borrowers) seek larger amounts?
...
```{r}
ggplot(loans, aes(x=status, y=funded_amount)) + geom_boxplot()
```
